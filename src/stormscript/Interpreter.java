...
//> Functions interpreter-constructor
    Interpreter() {
        globals.define("clock", new StormScriptCallable() {
            @Override
...
//> Statements and State interpret
    void interpret(List<Stmt> statements) {
        try {
            ...
            StormScript.runtimeError(error);
        }
    }
    //< Statements and State interpret
//> evaluate
...
//> Inheritance interpret-superclass
        ...
        if (stmt.superclass != null) {
            ...
            if (!(superclass instanceof StormScriptClass)) {
...
//> interpret-methods
        ...
        Map<String, StormScriptFunction> methods = new HashMap<>();
        for (Stmt.Function method : stmt.methods) {
/* Classes interpret-methods < Classes interpreter-method-initializer
      LoxFunction function = new LoxFunction(method, environment);
*/
//> interpreter-method-initializer
            StormScriptFunction function = new StormScriptFunction(method, environment,
...
//> Inheritance interpreter-construct-class
        StormScriptClass klass = new StormScriptClass(stmt.name.lexeme,
                (StormScriptClass)superclass, methods);
...
//> Classes construct-function
        StormScriptFunction function = new StormScriptFunction(stmt, environment,
                false);
//< Classes construct-function
...
//> check-is-callable
        if (!(callee instanceof StormScriptCallable)) {
            throw new RuntimeError(expr.paren,
                    "Can only call functions and classes.");
        }

//< check-is-callable
        StormScriptCallable function = (StormScriptCallable)callee;
//> check-arity
...
//> Classes interpreter-visit-get
    ...
    public Object visitGetExpr(Expr.Get expr) {
        ...
        if (object instanceof StormScriptInstance) {
            return ((StormScriptInstance) object).get(expr.name);
        }
...
//> Classes interpreter-visit-set
    ...
    public Object visitSetExpr(Expr.Set expr) {
        ...
        if (!(object instanceof StormScriptInstance)) { // [order]
            throw new RuntimeError(expr.name,
                    "Only instances have fields.");
        }

        Object value = evaluate(expr.value);
        ((StormScriptInstance)object).set(expr.name, value);
        return value;
    }
    //< Classes interpreter-visit-set
//> Inheritance interpreter-visit-super
    ...
    public Object visitSuperExpr(Expr.Super expr) {
        ...
        StormScriptClass superclass = (StormScriptClass)environment.getAt(
                distance, "super");
//> super-find-this

        StormScriptInstance object = (StormScriptInstance)environment.getAt(
                distance - 1, "this");
//< super-find-this
//> super-find-method

        StormScriptFunction method = superclass.findMethod(expr.method.lexeme);
//> super-no-method
